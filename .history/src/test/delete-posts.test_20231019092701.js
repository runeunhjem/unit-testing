...
describe("deletePost", () => {
  ...
  it("should handle network errors", async () => {
    ...
    localStorage.setItem("authorName", "loggedInUser"); // Simulate a logged-in user

    const postId = "789";

    // Use a try...catch block to capture the expected network error
    try {
      await deletePost(postId);

      // If the function doesn't throw an error, fail the test
      fail("Expected deletePost to throw a network error.");
    } catch (error) {
      // Check if the error is an instance of the Error class
      expect(error).toBeInstanceOf(Error);

      // Check the error message (modify the message to match the expected message)
